...
public class UndecoratorController {
	...
	private static UndecoratorController userHome = null;
	...
	public static UndecoratorController getInstance(String source1) {
		source = source1;
		return userHome;
	}
	...
	public void close() {
		...
		Platform.runLater(new Runnable() {
			...
			@Override
			public void run() {
				if (source == null || source.equals("close")) {
					CommonFunctions.message = "Please Confirm to Close the System...!";
				...
				} else if (source.equals("menuitem")) {
					stage.close();
					source = null;
				}
			}
	...
	public void setAsStageDraggable(final Stage stage, final Node node) {
		...
		node.setOnMouseClicked(new EventHandler<MouseEvent>() {
			...
			@Override
			public void handle(MouseEvent mouseEvent) {
				if (undecorator.getStageStyle() != StageStyle.UTILITY && !stage.isFullScreen() && stage.isResizable()
						&& mouseEvent.getClickCount() > 1) {
					if (mouseEvent.getSceneY() - SHADOW_WIDTH < MAXIMIZE_BORDER) {
						undecorator.maximizeProperty().set(!undecorator.maximizeProperty().get());
						mouseEvent.consume();
